...
async function manifest(entry){if(BookManifestRegistry.get(entry.id))return BookManifestRegistry.get(entry.id);await script(`./books/${entry.folder}/manifest.js?v=multi-book-8`);return BookManifestRegistry.get(entry.id);}
